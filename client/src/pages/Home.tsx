...
const CONDUCT_URL = "https://github.com/FriskyDevelopments/open-stay-pass/blob/main/CODE_OF_CONDUCT.md";
const LICENSE_URL = "https://github.com/FriskyDevelopments/open-stay-pass/blob/main/LICENSE";
const NETLIFY_URL = "https://www.netlify.com";

export default function Home() {
  const [locale, setLocale] = useState<Locale>("en");
  const supportLinks = getSupportLinks();
  const repositoryUrl = getGitHubRepositoryUrl() || "https://github.com/FriskyDevelopments/open-stay-pass";
  return (
    <main className="landing">
      <header className="public-nav">
        <div className="brand-combo"><i aria-hidden="true">H×F</i><span>HOSTCASA</span><b>×</b><span>FOLIOS</span></div>
        <div className="nav-right"><Link href="/press-kit">{copy(locale, "Press kit", "Press kit")}</Link><Link href="/integrations">{copy(locale, "Integraciones", "Integrations")}</Link><a href={CONDUCT_URL} target="_blank" rel="noreferrer">{copy(locale, "Código de conducta", "Code of Conduct")}</a><LanguageToggle locale={locale} onChange={setLocale} /></div>
      </header>
      ...
      <footer className="landing-footer">
        <span>HOSTCASA · Hospitality, Elevated.</span>
        <span>FOLIOS · ONE RECORD</span>
        <span>QR · WALLET · NFC · AI</span>
        <a className="support-link" href={repositoryUrl} target="_blank" rel="noreferrer"><Github size={14} /> {copy(locale, "Da una Star al repositorio", "Star the repository")}</a>
        <a className="support-link" href={CONDUCT_URL} target="_blank" rel="noreferrer">{copy(locale, "Código de conducta", "Code of Conduct")}</a>
        <a className="support-link" href={LICENSE_URL} target="_blank" rel="noreferrer">MIT</a>
        <a className="support-link" href={NETLIFY_URL} target="_blank" rel="noreferrer">{copy(locale, "Este sitio funciona con Netlify", "This site is powered by Netlify")}</a>
        {supportLinks.length ? <span className="support-links"><Heart size={14} /> {copy(locale, "Apoya el núcleo abierto", "Support the open core")}{supportLinks.map(link => <a key={link.id} href={link.href} target="_blank" rel="noreferrer">{link.label[locale]}</a>)}</span> : null}
      </footer>
    </main>
  );
}
